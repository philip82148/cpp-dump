...
#define CPP_DUMP_EXPAND_FOR_EXPORT_ENUM_(item) \
  { item, #item }
#define CPP_DUMP_DEFINE_EXPORT_ENUM(type, ...)                                                   \
  ...
  std::string export_var(const T &, const std::string &, size_t, size_t, bool);                  \
                                                                                                 \
  template <>                                                                                    \
  inline std::string export_enum(const type &value, const std::string &, size_t, size_t, bool) { \
    std::map<type, std::string> enum_map{                                                        \
        CPP_DUMP_EXPAND_VA_(CPP_DUMP_EXPAND_FOR_EXPORT_ENUM_, __VA_ARGS__)};                     \
    return enum_map.count(value) ? enum_map[value] : #type "::?";                                \
  }                                                                                              \
                                                                                                 \
  } /* namespace _detail */                                                                      \
                                                                                                 \
