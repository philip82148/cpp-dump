...
| Ostream      | All of the above are not satisfied and the function `std::ostream& operator<<(std::ostream&, const T &)` is defined. If the string representation of T is an empty string, export_var() returns 'manipulators'. |                                                   |
